"""Turn the text box into context and questions.
...
MAX_OPTIONS = 32     # a limit of the demo; the library takes as many as the model has single-token indexes
MAX_QUESTIONS = 10   # each question is its own paid request on the shared key
# Not the singular "option:" or "choice:", which also start ordinary context lines.
CHOICES_RE = re.compile(r"^\s*(choices|chocies|choises|options)\s*:\s*(.*)$", re.IGNORECASE)
...
def parse_options(spec: str, line: int) -> list[Option]:
    ...
    if len(options) > MAX_OPTIONS:
        raise ParseError(f"line {line}: at most {MAX_OPTIONS} choices per question (the answer is one token)", line)
    seen: set[str] = set()
    for o in options:
        key = o.name.lower()
        if key in seen:
...
def parse(text: str) -> Query:
    ...
    for i, raw in enumerate(lines):
        ...
        if len(questions) == MAX_QUESTIONS:
            raise ParseError(f"line {j + 1}: at most {MAX_QUESTIONS} questions at a time", j + 1)
