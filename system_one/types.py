...
``SystemOneResponse``, ``Usage``) so code written against the upstream
library keeps working, without taking the dependency: this package talks
...
class SystemOneResponse:
    ...
    def choices(self) -> dict[str, ChoiceAnswer]:
        """Upstream calls this ``choices``; ``answers`` is the same map."""
        return self.answers
