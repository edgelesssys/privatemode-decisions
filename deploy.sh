...
#   HOST=myhost DIR=privatemode-system-one sh deploy.sh
set -eu
HOST="${HOST:?set HOST to the ssh host}"; DIR="${DIR:-privatemode-system-one}"
cd "$(dirname "$0")"
.venv/bin/pytest -q tests
git ls-files > /tmp/privatemode-system-one.files
ssh "$HOST" "mkdir -p $DIR"
rsync -az --files-from=/tmp/privatemode-system-one.files ./ "$HOST:$DIR/"
