...
Kept separate from :mod:`decisions.inference` because it is the only part
...
def to_data_url(source: Any, *, max_side: int | None = None, quality: int = 80) -> str:
    ...
    ``source`` may be a data URL, an ``http(s)`` URL, a path, raw bytes, or
    a Pillow image. ``max_side`` scales the longest edge down to that many
    pixels -- never up, since inventing pixels only costs tokens. An
    ``http(s)`` URL is returned unchanged, since the server fetches it, and
    so is a data URL when there is no ``max_side``.

    Any other string is read as a file path, and an ``http(s)`` URL is
    fetched by the server: don't pass strings from untrusted users here.
    """
    ...
    if isinstance(source, str) and source.startswith("data:"):
        ...
        if not header.endswith(";base64"):
            raise ValueError("Only base64 data URLs can be resized")
        return _encode(_open(base64.b64decode(payload)), max_side, quality)
